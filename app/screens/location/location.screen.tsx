...
const LocationScreen = (props: LocationScreenProps) => {
  return (
    <SafeAreaView style={locationStyle.flex}>
      <HeaderComponent
        title="Incident Location Screen"
        hasBackButton={true}
        navigation={props.navigation}
      />
      <View style={locationStyle.flex}>
        <MapView
          initialRegion={{
            latitude: 53.470779,
            longitude: -2.24128,
      ...
      <Card>
        <Card.Title
          title={"29/08/2023"}
          titleStyle={locationStyle.cardTitle}
          right={() => (
            <Text style={locationStyle.cardTitle}>Distance: 0.3km</Text>
          )}
        />
        <Card.Content>
          <List.Item
            title={"Ejemen Omo-osagie"}
            description="31 Incident Cases"
            left={() => (
              <Avatar.Image
                size={52}
                source={{
